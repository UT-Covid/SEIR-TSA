...
get_mobility_data = function () {
  ...
  visits = read_csv("./data/visits_by_category_bycounty_baselined.csv")[ -1]
  ...
  msa_data = county_poi %>%
    ...
    group_modify(~ { # add 7-week average
      ...
      new_cols = paste0(cur_cols, "_av7")
      df[ ,new_cols] = NA
      for (i in 7:nrow(df))
        df[i, new_cols] = apply(df[(i - 7):i, cur_cols], 2, mean)
    ...
    msa_data[ , paste0("PC", 1:3, "_av7")] = comps
