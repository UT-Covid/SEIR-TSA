...
get_mobility_data = function (average_mode=c("trailing", "centered"), days_average=7) {
  
  if (length(average_mode) > 1)
    average_mode = average_mode[1]

  ## read latest mobility data
  visits = read_csv("./data/visits_by_category_bycounty_baselined_extra.csv")[ -1]
  sdmetrics = read_csv("./data/sd-metrics_bycounty_baselined.csv")[ ,-1]
  ...
  msa = read_csv("./data/csa.csv") %>% 
    ...
    left_join(group_by(., msa) %>% 
      ...
      by="msa") %>%
    filter(pop_rank <= 300) %>%  # keep only largest 300 araas
  ...
  top_categories <- c(
    ...
    "Gasoline Stations"="gas_stations",
    "Building Material and Supplies Dealers"="building_materials",
    "General Merchandise Stores, including Warehouse Clubs and Supercenters"="stores",
    "Other Amusement and Recreation Industries"="other_amusement",
    "Continuing Care Retirement Communities and Assisted Living Facilities for the Elderly"="ltc_facilities",
    "Nursing Care Facilities (Skilled Nursing Facilities)"="nursing_facilities"
    )
  ...
  msa_data = county_poi %>%
    ...
    group_modify(~ { # add 7-week average
      ...
      cur_cols = c(paste0("count_pc_", unname(top_categories)), sd_cols)
      ...
      for (i in days_average:nrow(df)) {
        ...
        end_index = i
        df[i - offset, new_cols] = apply(df[start_index:end_index, cur_cols], 2, mean, na.rm=TRUE)
      }
